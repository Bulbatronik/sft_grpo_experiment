...
#   GRPO_CONFIG  path to GRPO config YAML   (default: configs/grpo_<MODEL_NAME>.yaml)
...
[ -f "$GRPO_CONFIG" ] || GRPO_CONFIG="$REPO_DIR/configs/base_grpo.yaml"

# ── Map array task index → (SFT checkpoint, GRPO data selection) ──────────────
SFT_SELS=("diverse_10pct" "random_10pct" "diverse_20pct" "random_20pct")
GRPO_SELS=("variance_10pct" "random_10pct" "variance_20pct" "random_20pct")
...
echo "Array task $IDX: SFT=$SFT_SEL  GRPO=$GRPO_SEL  MODEL=$MODEL_NAME  CONFIG=$(basename $GRPO_CONFIG)"
